...
if __name__ == "__main__":
    ...
    gallery_content = "### [<-返回首页]({0}) \n  \n## {1}  \n  \n".format(config['URL1'], config['title'])
        ...
        if current_folder == "/":
            ...
            for pic in allimgfiles:
                ...
                gallery_content = gallery_content + "### {0}  \n![{0}]({1}{2}/{0})  \n  \n".format \
                    (pic.name, config['URL2'], config['gallery'])
                # FTP_upload(pic, config)
                p.apply_async(FtpOp.FTP_upload, (pic, config), )  # 多进程入口
            p.close()
            p.join()
    ...
    gallery_index_file.writelines(gallery_content)
    ...
    photobook_index_file.writelines(content)
